...
import com.dominion.card.AttackCard;
import com.dominion.card.ConditionalVictoryCard;
import com.dominion.card.InteractingCard;
import com.dominion.card.ReactionCard;
import com.dominion.card.VictoryCard;
...
public class Game implements StreamListener, Runnable {
	...
	public class PlayerInfo {
		...
		//Caller's responsibility to ensure dominion.card is gainable
...
//				System.out.println("Server: Player " + i + " should have reacted? " + react);
		...
		public void sendPlay(ActionCard c) {
			for(PlayerInfo pi : Game.this.players)
				pi.streams.sendMessage(new RemoteMessage(RemoteMessage.Action.playCard, playerNum, c, null));
		}

		public void sendDeckReveal(Card c) {
			for(PlayerInfo pi : Game.this.players)
				pi.streams.sendMessage(new RemoteMessage(RemoteMessage.Action.revealFromDeck, playerNum, c, null));
		}

		public void sendHandReveal(Decision.CardListDecision cld) {
			for(PlayerInfo pi : Game.this.players)
				pi.streams.sendMessage(new RemoteMessage(RemoteMessage.Action.revealFromHand, playerNum, null, cld));
		}

		public void sendGain(Card c) {
			for(PlayerInfo pi : Game.this.players)
				pi.streams.sendMessage(new RemoteMessage(RemoteMessage.Action.gainCard, playerNum, c, null));
		}

		public void sendShuffled() {
			for(PlayerInfo pi : Game.this.players)
				pi.streams.sendMessage(new RemoteMessage(RemoteMessage.Action.cardsWereShuffled, playerNum, null, null));
		}
		private void sendEndTurn() {
			for(PlayerInfo p : players)
				p.streams.sendMessage(new RemoteMessage(RemoteMessage.Action.endTurn, playerNum, null, null));
		}

		public void sendPutOnDeck(Card c) {
			for(PlayerInfo pi : Game.this.players)
				pi.streams.sendMessage(new RemoteMessage(RemoteMessage.Action.putOnDeck, playerNum, c, null));
		}

		public void sendPutOnDeckFromHand(Card c) {
			for(PlayerInfo pi : Game.this.players)
				pi.streams.sendMessage(new RemoteMessage(RemoteMessage.Action.putOnDeckFromHand, playerNum, c, null));
		}

		public void sendPutInHand(Card c) {
			for(PlayerInfo pi : Game.this.players)
				pi.streams.sendMessage(new RemoteMessage(RemoteMessage.Action.putInHand, playerNum, c, null));
		}

		public void sendDiscardCard(Card c) {
			for(PlayerInfo pi : Game.this.players)
				pi.streams.sendMessage(new RemoteMessage(RemoteMessage.Action.discardCard, playerNum, c, null));
		}

		public void sendDiscardCardList(Decision.CardListDecision cld) {
			for(PlayerInfo pi : Game.this.players)
				pi.streams.sendMessage(new RemoteMessage(RemoteMessage.Action.discardCardList, playerNum, null, cld));
		}
...
//			nextTurn.inHand.clear();
		...
		private void sendCardToHand(Card c) {
			RemoteMessage rm = new RemoteMessage(RemoteMessage.Action.addCardToHand, playerNum, c, null);
			System.out.println("Server: sending dominion.card to player " + rm);
			//TODO send to everyone that you got a dominion.card
		...
		public Card getPlay() {
			streams.sendMessage(new RemoteMessage(RemoteMessage.Action.chooseAction, playerNum, null, null));
			RemoteMessage m = null;
			while(m == null || m.action != RemoteMessage.Action.playCard || m.playerNum != playerNum) {
				try {
				...
				if(m!=null && (m.action != RemoteMessage.Action.playCard || m.playerNum != playerNum))
		...
		public List<Card> getBuys(int upperLimit, int numGains) {
			streams.sendMessage(new RemoteMessage(RemoteMessage.Action.chooseBuy, playerNum, null, new Decision.GainDecision(upperLimit, numGains)));
			RemoteMessage m = null;
			while(m == null || m.action != RemoteMessage.Action.buyCards || m.playerNum != playerNum) {
				try {
					m = Game.this.messageQ.take();
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
				...
				if(m!=null && (m.action != RemoteMessage.Action.buyCards || m.playerNum != playerNum))
					System.out.println("was expecting a buyCards from player "+ playerNum + ", got message: " + m);
			}
			return ((Decision.CardListDecision)m.decisionObject).list;
		}

		// d will often be null, this param is to allow the dominion.card to give info as to which
		// decision needs to be made in the case that there are multiple decisions
		// to be made for this dominion.card.
		public Decision getDecision(Card c, Decision d) {
			streams.sendMessage(new RemoteMessage(RemoteMessage.Action.makeDecision, playerNum, c, d));
			RemoteMessage m = null;
			while(m == null || m.action != RemoteMessage.Action.sendDecision || m.playerNum != playerNum) {
				try {
					m = Game.this.messageQ.take();
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
				...
				if(m!=null && (m.action != RemoteMessage.Action.sendDecision || m.playerNum != playerNum))
...
//		randomizerDeck.addAll(Arrays.asList(Card.alchemyRandomizerDeck));
	...
	private void sendStacks() {
		RemoteMessage rm = new RemoteMessage(RemoteMessage.Action.stack, -1, null, new Decision.StackDecision(stacks));
	...
	public void sendScores(ScoresObject scores) {
		...
		RemoteMessage rm = new RemoteMessage(RemoteMessage.Action.endScore, -1, null, scores);
		for(PlayerInfo pi : Game.this.players)
			pi.streams.sendMessage(rm);
	}
