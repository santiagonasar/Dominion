...
public class ServerTurn extends Turn {
	...
	public void takeTurn() {
		...
		List<Card> buys;
		do {
			//prompt to buy a dominion.card or choose not to
	...
	private void sendConfirmDecision(Decision d, Card inProgress) {
		player.streams.sendMessage(new RemoteMessage(RemoteMessage.Action.sendDecision, player.playerNum, inProgress, d));
	}

	private boolean checkCardListInHand(Decision.CardListDecision cld) {
	...
	//dominion.card must verify appropriate number
	public boolean trashCardsFromHand(Decision.CardListDecision cld, Card inProgress) {
	...
	//dominion.card must verify appropriate number
	public boolean discardCardsFromHand(Decision.CardListDecision cld, Card inProgress) {
	...
	public void discardHand() {
		...
		player.sendDiscardCardList(new Decision.CardListDecision(new ArrayList<Card>(inHand)));
	...
	public void revealHand() {
		player.sendHandReveal(new Decision.CardListDecision(inHand));
	}
	...
	// but i'm intending this to be used when you need to "look at" a dominion.card
	...
	// add a dominion.card to the hand from the supply, use gainCardToHand
	...
	//Note: caller must verify presence of dominion.card
	@Override
	...
	public void sendDecisionToPlayer(Card c, Decision d) {
		player.streams.sendMessage(new RemoteMessage(RemoteMessage.Action.sendDecision, player.playerNum, c, d));
	}
