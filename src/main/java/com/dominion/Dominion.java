...
import com.dominion.Game.ScoresObject;
import com.dominion.card.ActionCard;
import com.dominion.card.DecisionCard;
...
//		playFrame.setLocation(handFrame.getSize().width, supplyFrame.getSize().height);
	...
	private RemoteMessage.Action getActionForState() {
		switch(state) {
			case waitingForBuy: return RemoteMessage.Action.buyCards;
			case waitingForGain: return RemoteMessage.Action.sendDecision;
			default: return null;
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		case waitingForAction: 
			if(act.equals("No More Actions"))
				rm = new RemoteMessage(RemoteMessage.Action.playCard, localPlayer, null, null);
			else if(e.getSource() instanceof HandCardButton)//act.startsWith("Play "))
				rm = new RemoteMessage(RemoteMessage.Action.playCard, localPlayer, ((HandCardButton)e.getSource()).card, null);
			else
				break;
		...
		case waitingForSelectFromHand:
			if(act.equals("Submit Selection"))
			{
				...
				rm = new RemoteMessage(RemoteMessage.Action.sendDecision, localPlayer, null, dec);
				streams.sendMessage(rm);
				state = GameState.none;
				message.setText("");
				fromHand.clear();
...
//							if(boxNum >= numPlayers)
	...
	public void setupCardSelection(int upperLimit, boolean exact, SelectionType type, SelectionCard c) {
		String amount = (upperLimit == -1) ? "any number of cards":
			(((exact) ? "exactly " : "at most ") + upperLimit + " dominion.card(s)");
...
//		TODO enforce how many more you MUST and/or CAN select with GUI
	...
	public void setupGainCard(int upperLimit, boolean exact, SelectionCard c, String s) {
		if(s == null) {
			...
			message.setText("Choose a dominion.card costing " + amount + " to gain.");
		} else {
			message.setText(s);
		}
...
//			"\"></img></html>" + c;
	...
	public void recieveMessage(RemoteMessage m) {
		...
		switch(m.action) {
		case stack:
			stacks = ((Decision.StackDecision)m.decisionObject).stacks;
			setupSupplyGUI();
			break;
		...
		case revealFromHand:
			...
			break;
		case putOnDeck:
			messageText += names[m.playerNum] + " put dominion.card on deck: " + getImageLinkForCard(m.card) +  ".\n";
			messagePane.setText(messageText);
			//TODO visual display
			break;
		case discardCard:
			messageText += names[m.playerNum] + " discarded dominion.card: " + getImageLinkForCard(m.card) +  ".\n";
			messagePane.setText(messageText);
			//TODO visual display
			break;
		case discardCardList:
			messageText += names[m.playerNum] + " discarded cards: " + ((Decision.CardListDecision) m.decisionObject).list + ".\n";
			messagePane.setText(messageText);
			if(m.playerNum == localPlayer)
				for(Card c : ((Decision.CardListDecision) m.decisionObject).list)
					discardCard(m.playerNum, c);
			//TODO visual display
			break;
		case putOnDeckFromHand:
			messageText += names[m.playerNum] + " put dominion.card on deck from hand: " + getImageLinkForCard(m.card) +  ".\n";
			messagePane.setText(messageText);
			if(m.playerNum == localPlayer) undrawCard(m.playerNum, m.card);
			break;
		case putInHand:
			messageText += names[m.playerNum] + " put dominion.card into hand: " + getImageLinkForCard(m.card) +  ".\n";
			messagePane.setText(messageText);
		...
		case addCardToHand:
			...
			if(m.playerNum == localPlayer){
				...
				messageText += names[m.playerNum] + " drew a dominion.card.\n";
				messagePane.setText(messageText);
			}
			break;
		case chooseAction: 
		...
		case chooseBuy: 
			if(m.playerNum == localPlayer) {				
				message.setText("May buy " + ((Decision.GainDecision)m.decisionObject).numGains + " item(s) costing a total of " + ((Decision.GainDecision)m.decisionObject).upperLimit + ".");
				state = GameState.waitingForBuy;
				fromSupply.clear();
				playerModels[localPlayer].turn.buyingPower = ((Decision.GainDecision)m.decisionObject).upperLimit;
				playerModels[localPlayer].turn.numBuysLeft = ((Decision.GainDecision)m.decisionObject).numGains;
				enableButtons(((Decision.GainDecision)m.decisionObject).upperLimit, null);
			}
			break;
		case endTurn: 
			endTurn(m.playerNum); 
	...
	public <E extends Enum<E>> void makeMultipleChoiceDecision(String text, Class<E> enumType, Card c) {
		...
		Decision.EnumDecision<E> ed = new Decision.EnumDecision<E>(options[result]);
		streams.sendMessage(new RemoteMessage(RemoteMessage.Action.sendDecision, localPlayer, null, ed));
	}
